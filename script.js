document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ========================================
     1. MATRIX ENGINE (Musi startować ZAWSZE, jako tło)
     ======================================== */
  ...
  if (canvas) {
    ...
    function matrix(currentTime) {
      ...
      // Slight fade effect for trail (niższa opacity dla lepszej widoczności w maintenance)
  ...
  /* ========================================
     ...
     ======================================== */
  const CONFIG = {
    maintenanceMode: true, // Zmień na true, aby włączyć tryb konserwacji
    enablePreloader: true, // Zmień na false, aby wyłączyć preloader
  };

  /* ========================================
     3. PRELOADER LOGIC (Smart Mask)
     ======================================== */
  if (CONFIG.enablePreloader) {
    const preloader = document.getElementById("preloader");
    if (preloader) {
      // Ukryj preloader, gdy strona jest w pełni załadowana (obrazki, style)
      window.addEventListener("load", () => {
        // Minimalne opóźnienie 500ms dla efektu "płynności", max 2s (timeout bezpieczeństwa)
        setTimeout(() => {
          preloader.classList.add("loaded");
          // Usuń z DOM po animacji, żeby nie blokował kliknięć
          setTimeout(() => {
      ...
      // Timeout bezpieczeństwa (jeśli coś się zatnie, ukryj po 3s)
      setTimeout(() => {
    ...
    // Jeśli wyłączony w configu, ukryj natychmiast
    const preloader = document.getElementById("preloader");
  ...
  /* ========================================
     ...
     ======================================== */
  // Sprawdź czy w URL jest ?admin=true (Bypass)
  const urlParams = new URLSearchParams(window.location.search);
  const isAdmin = urlParams.get("admin") === "true";

  if (CONFIG.maintenanceMode && !isAdmin) {
    const maintenanceScreen = document.getElementById("maintenance-screen");
    const siteContent = document.getElementById("site-content"); // Nowy wrapper

    if (maintenanceScreen && siteContent) {
      // Pokaż ekran serwisowy
      maintenanceScreen.classList.add("active");
      // UKRYJ CAŁĄ RESZTĘ STRONY (To rozwiązuje problem nakładania)
      siteContent.style.display = "none";
      // Zablokuj scroll
      document.body.style.overflow = "hidden";
      return; // Zatrzymaj resztę skryptów (Czat, Slidery itp. nie są potrzebne)
    }
  }
      ...
      if (suggestedBudget !== undefined) {
        ...
        if (suggestedBudget === 0) {
          ...
          budgetValue.innerText =
            suggestedBudget.toLocaleString("pl-PL") + " PLN";
        }
      ...
      setTimeout(() => {
        ...
        if (formMessage) {
          formMessage.innerText =
            "Dzięki! Wiadomość wysłana. Odezwę się wkrótce.";
  ...
  if (
    ...
    const interactiveSelectors =
      "a, button, input, textarea, select, .project-card, .service-card, .chatbot-trigger, .progress-wrap, input[type='range'], input[type='checkbox'], .cert-chip";
