...
const VALIDATION_MESSAGES = {
  businessGoal: {
    required: "Opisz cel biznesowy swojego projektu.",
    min: (n) => "Opis celu jest zbyt krótki " + "(min. " + n + " znaków)."
  },
  audience: {
    required: "Zdefiniuj grupę docelową " + "(Twoich klientów).",
    min: (n) => "Opis grupy jest zbyt krótki " + "(min. " + n + " znaków)."
  },
  default: {
    required: "To pole jest wymagane.",
    min: (n) => "Wymagane jest minimum " + n + " znaków."
  }
};
...
function initPreloaderMatrix() {
  ...
  function matrixLoop(currentTime) {
    ...
    ypos.forEach((y, ind) => {
      const chars = "01DOMINDEVBRIEFXYZ";
...
async function initBriefApp() {
  ...
  if (params.get("demo") === "1") {
    ...
    briefToken = "demo";
    briefSig = "demo";
    const emailEl = document.getElementById("brief-email");
  ...
  briefToken = params.get("t");
  briefSig = params.get("sig");
  ...
  try {
    const res = await fetch(
      `/api/brief-meta?t=${encodeURIComponent(briefToken)}&sig=${encodeURIComponent(briefSig)}`
    );
    const data = await res.json();

    if (!data.ok) {
      if (data.errorCode === "TOKEN_USED") {
        showState("used");
      } else {
        showState("expired");
      }
...
function setupHints() {
  document.querySelectorAll('.hint-trigger').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.hint-content.active').forEach(h => h.classList.remove('active'));
      document.querySelectorAll('.hint-trigger.active').forEach(b => b.classList.remove('active'));
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.hint-content')) {
      document.querySelectorAll('.hint-content.active').forEach(h => h.classList.remove('active'));
      document.querySelectorAll('.hint-trigger.active').forEach(b => b.classList.remove('active'));
    }
  });
...
function goToStep(step) {
  ...
  document.querySelectorAll(".step-panel").forEach((panel) => {
    const panelStep = parseInt(panel.id.replace("panel-", ""));
...
function validateStep(step) {
  ...
  fields.forEach((field) => {
    const msgContainer = field.closest(".form-group").querySelector(".validation-msg");
    const minLen = parseInt(field.getAttribute("minlength") || "0");
    const value = field.value.trim();
    const fieldName = field.name;

    let error = "";
    if (!value) {
      error = (VALIDATION_MESSAGES[fieldName]?.required) || VALIDATION_MESSAGES.default.required;
    } else if (value.length < minLen) {
      error = (VALIDATION_MESSAGES[fieldName]?.min?.(minLen)) || VALIDATION_MESSAGES.default.min(minLen);
    }
...
// --- Autosave ---
const DRAFT_KEY = () => `briefDraft:${briefToken}`;
...
function setupAutosave() {
  ...
  form.addEventListener("input", (e) => {
    ...
    if (e.target.classList.contains("invalid")) {
      ...
      const msg = e.target.closest(".form-group").querySelector(".validation-msg");
...
function setupClearDraft() {
  ...
  btn.addEventListener("click", () => {
    if(!briefToken) return;
    if (confirm("Czy na pewno chcesz usunąć zapisany szkic?")) {
      localStorage.removeItem(DRAFT_KEY());
      ALL_FIELDS.forEach((key) => {
        const el = document.querySelector(`[name="${key}"]`);
        if (el) el.value = "";
...
function buildSummary() {
  ...
  container.innerHTML = Object.entries(FIELD_LABELS)
    .map(([key, label]) => {
      ...
      const isRequired = ["businessGoal", "audience"].includes(key);
      return `<div class="summary-item">
        <dt>${escapeHtml(label)}${isRequired ? " *" : ""}</dt>
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if(errorEl) errorEl.textContent = data.message || "Wystąpił błąd. Twoje odpowiedzi są zapisane — spróbuj ponownie.";
        showState("error");
      }
    } catch (err) {
      console.error("Submit failed:", err);
      const errorEl = document.getElementById("error-message");
      if(errorEl) errorEl.textContent = "Problem z połączeniem. Twoje odpowiedzi są zapisane — spróbuj ponownie.";
      showState("error");
    }
  });
}
